...
var ratio = parseInt(document.getElementById("html").getAttribute("data-dpr"))/2<1.5?1:1.5;
...
function create(){
    ...
    for(var i = 0;i<7;i++){
        if(i<3){
            ...
            p.position.x = (winWidth-p.width*4)/2+i*p.width+3*i*(p.width*4/3-p.width)/2;
...
function moveOut(e){
    ...
    var newTime = game.time.events.add(Phaser.Timer.SECOND * r, function(){
        ...
        if(moveIndex<4){
            ...
            p_b.position.x = (winWidth-120*ratio*4)/2+((i)*p_b.width)-(15*ratio);
            ...
            p_b.position.y = rectangle.height+rectangle.top- p_b.height-100+160*ratio-3;
...
function dispose(e,b){
    ...
    p_h.positionIndex = b;
    p_h.name = k;
    p_h.position.x = (winWidth-120*ratio*4)/2+a*p_h.width+3*a*(160*ratio-p_h.width)/2;
...
function generate(i){
    ...
    p_b.scale.set(ratio,ratio);
    if(index<4){
        p_b.position.x = (winWidth-120*ratio*4)/2+((index)*150*ratio)-(15*ratio);
        p_b.position.y = rectangle.height+rectangle.top- p_b.height+160*ratio-6;
