...
function judgeDrop(e){//判断是否进入矩形线框中
    ...
    if(
        e.y>toTop/2
        &&e.y<rectangleHeight+toTop-headHeight/2
...
function shoot(){
    ...
    //var photo1 = bmd1.copy(bmd_draw, (winWidth-rectangleWidth)/2, 0, rectangleWidth, rectangleWidth,0,0, rectangleWidth, rectangleWidth);

    $(".container").removeClass("game").addClass("result");
    $("#resultCanvas").append(photo.canvas);
    $("#resultCanvas canvas").css("-webkit-transform","scale("+(winWidth*5/8)/590+","+(winWidth*5/8)/590+")");
...
function creatImg(c){
    ...
    $.ajax({
        ...
        success: function(data) {
            var img = new Image();
            img.src = data.path+"!thumb";
            img.onload = function(){
                window.shareData["imgUrl"] = data.path+"!thumb";
            }
        }
    });
}
